...
class _StepInfoDict(dict[str, Any]):
    # SB3 копирует info каждый шаг, shallow ок
...
class BalloonEnv(gym.Env):
    # один интерполятор на одну среду
    ...
    def consume_step_record(self) -> dict[str, Any]:
        # в info только скаляры для tb; шаги на диск пишет _steps_writer в воркере
    ...
    def get_trajectory_viz_state(self) -> dict[str, Any]:
        # мета траекторий для снапшота рендера
    ...
    def _trim_episode_history(self) -> None:
        # файлы не трогаем — рендер читает асинхронно; чистка в cleanup_steps_dir
