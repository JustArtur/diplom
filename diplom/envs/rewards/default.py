"""Reward ``default`` — полная функция награды (baseline).

CLI: ``--reward default``

Экспорт для среды
-----------------
- ``WIND_ALIGN_SCALE = 20.0`` — нормализация wind_toward в obs.
- ``Z_STICK_WINDOW_STEPS = 50_000`` — окно z_stick (~50k шагов sim).

Вход
----
``compute_reward(wind_interp, step, ctx, state) -> RewardResult``

Ветер берётся из ``step.wind`` (текущая высота баллона). ``wind_interp`` не используется.

Термы награды (все включены)
----------------------------
1. **wind_align** — проекция ветра на направление к цели; масштабируется
   ``wind_progress_scale`` (0 при серии отриц. progress).
2. **wind_align_delta** — изменение wind_toward относительно прошлого шага.
3. **progress** — асимметричный XY/Z progress с zone multiplier (×3/×10/×25
   ближе к цели); pos_coef > neg_coef.
4. **distance** — слабый штраф за большую XY-дистанцию + квадратичный ближе 50 км.
5. **energy** — штраф за расход энергии на шаг.
6. **boundary** — штраф за contact с границей ERA5 / потолком.
7. **best_distance** — бонус при новом рекорде приближения по XY (< 50 км).
8. **distance_regression** — штраф за отход от best_horizontal_distance;
   coef=0.5, scale=1000 м.
9. **hold_close** — малый бонус при dist_xy < 5 км.
10. **wind_streak** / **wind_adverse_streak** — бонус/штраф за длинные серии
    favorable/adverse wind.
11. **wind_scan** — бонус за смену слоя (|dz|≥20 м) с улучшением wind_toward.
12. **adverse_wind_close** — штраф: близко к цели + встречный ветер.
13. **high_altitude** — штраф: Z > 5000 м + встречный ветер.
14. **idle_action** — штраф за «жмёт action, но не двигается по Z» 40+ шагов.
15. **z_stick** — штраф если std(Z) за 50k шагов < 200 м.

Завершение эпизода
------------------
- ``terminated``: dist_xy ≤ target_reach_radius И |ΔZ| ≤ target_vertical_reach_radius
  → ``reward += SUCCESS_REWARD (500)``.
- ``truncated``: step_count ≥ max_episode_steps.

Логирование
-----------
Каждый терм пишется в ``RewardResult.terms`` как ``reward_<name>_term`` и
попадает в TensorBoard через info callback.
"""
