"""Reward ``goal_only`` — sparse reward только за достижение цели.

CLI: ``--reward goal_only``

Идея
----
Никакого dense shaping: агент получает награду только в тот момент, когда
эпизод завершается успехом. Все промежуточные термы обнулены, но состояние
эпизода продолжает обновляться, чтобы не ломать obs-фичи и диагностические
метрики.
"""
