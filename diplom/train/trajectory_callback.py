...
class TrajectoryVisualizationCallback(BaseCallback):
    ...
    Для каждой среды создаётся ``env_XXX.html`` (live-viewer) и
    ``env_XXX_data.js`` (данные, обновляются каждый rollout). Откройте HTML
    в браузере — график обновляется автоматически, камера сохраняется.
